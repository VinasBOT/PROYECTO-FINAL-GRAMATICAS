...
def process_function_definition(ast, out):
    ...
    utils.functions[function_identifier] = utils.get_function_parameters(parameters_tree)
...
def write_relational_expression(left, operator, right, out):
    match operator:
        ...
        case "%":
            utils.variables.append("")
            out.write(f"\t%{len(utils.variables)} = srem i32 {left}, {right}\n")
