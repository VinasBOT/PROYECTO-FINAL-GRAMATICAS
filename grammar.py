grammar = """
    start: (function_definition)* main_function*

    main_function: "function" "main" "(" ")" "{" main_function_block "}"
    function_definition: "function" identifier "(" parameters? ")" "{" block "}"

    main_function_block: (assignment | conditional | loop)*
    block: (assignment | conditional | loop)* statement?
    ...
    RELATIONAL_OPERATOR: ("<" | "<=" | ">" | ">=" | "==" | "=/=" | "%")
    relational_expression: (identifier | NUMBER | "(" relational_expression ")") RELATIONAL_OPERATOR (identifier | NUMBER | "(" relational_expression ")" | function_call)
